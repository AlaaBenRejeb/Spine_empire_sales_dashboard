...
evaluated_rows AS (
  SELECT
    c.*,
    CASE
      WHEN c.base_state <> 'candidate' THEN c.base_state
      WHEN EXISTS (
        SELECT 1
...
FROM (
  SELECT
    'candidate_counts'::text AS report,
    final_state,
    target_kind,
    actor_role,
    to_status,
    coalesce(timestamp_basis, 'n/a') AS timestamp_basis,
    COUNT(*) AS row_count
  FROM evaluated_rows
  GROUP BY final_state, target_kind, actor_role, to_status, coalesce(timestamp_basis, 'n/a')

  UNION ALL

  SELECT
    'skip_counts'::text AS report,
    final_state,
    target_kind,
    actor_role,
    to_status,
    ...
    COUNT(*) AS row_count
  FROM evaluated_rows
  WHERE final_state <> 'insert_candidate'
  GROUP BY final_state, target_kind, actor_role, to_status, coalesce(timestamp_basis, 'n/a')
) AS report_rows
ORDER BY report, final_state, actor_role, to_status, timestamp_basis, target_kind;
